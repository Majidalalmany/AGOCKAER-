  ...
  return (
    <header className="bg-white text-slate-800 border-b border-gray-200 sticky top-0 z-30 shadow-xs">
      <div className="flex items-center justify-between px-4 lg:px-8 h-16">
        ...
        {/* Right Section: Mobile menu toggle + Active tab indicator */}
        <div className="flex items-center gap-3">
          <button 
            onClick={onToggleSidebar}
            className="p-2 rounded-lg hover:bg-gray-100 transition-colors text-slate-600 lg:hidden"
            title="القائمة"
          >
          ...
          <div className="flex items-center gap-2.5">
            <h2 className="text-base font-bold text-slate-800 tracking-tight">جاهز</h2>
            <span className="text-gray-300 text-xs hidden sm:inline">|</span>
            <span className="text-blue-700 text-xs font-semibold bg-blue-50 px-3 py-1 rounded-md border border-blue-100">
              {activeTabLabel}
            </span>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Absher Support CTA Button */}
          {onOpenAbsherSupport && (
            <button
              onClick={onOpenAbsherSupport}
              className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-all shadow-2xs active:scale-98"
              title="فتح محادثة أبشر لطلب تعديل أو الإبلاغ عن مشكلة"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-300" />
              <span className="hidden md:inline">أبشر للتعديلات 💬</span>
            </button>
          )}
